...
require_once ('Sysgran/Aplicacion/Modulos/Produccion/Store/StoreMaquina.php');
...
class FormularioController extends JSonRouterBase {
	...
	private function ConfigurarListadoImpresion ($lst) {
		$lst->Store = new StoreMaquina ();
		$lst->NombreArchivo = 'listado';
		$lst->Titulo = 'LISTADO DE MAQUINAS';
		$lst->Columnas = array ('Codigo' => 'codigo', 'Descripcion' => 'descripcion');
		$lst->AnchoColumnas = array ('150', '500');
	}
	
	public function Crear ($centroDeTrabajoId, $codigo, $descripcion, $atributos) {
		global $conn;
	...
	public function BuscarPorCodigo ($codigo) {
		try {
			...
			$m = Maquina::GetMaquinaPorCodigo ($codigo);

			if ($m == null) {
				return Encoder::EncodeResponseError ('Codigo Inexistente');				

			} else {
				return Encoder::Encode (Array ($m));	
			}			
		} catch (Exception $ex) {
